"""Deploy settle/score sync fix to api.picks.matupicks.app."""
import os
import sys
import time
import paramiko
...
def main():
    ...
    run(
        c,
        f"cd {APP} && node -e \"require('dotenv').config(); require('./src/services/pickSettlementService').settlePendingPickResultsOnce().then(r=>console.log(JSON.stringify(r)))\"",
        180,
    )
